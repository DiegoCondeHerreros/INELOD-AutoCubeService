...
def detect_and_replace_measures(file_path, measure):
    ...
    medidas = rdflib.Graph().parse("inelod-voc-measure.ttl", format="ttl")

    # Get the measurement argument if provided
    print(f"Measurement to append: {measure}")
    with open(file_path, mode='r', encoding='utf-8', errors='replace') as csvfile:
        reader = csv.reader(csvfile, delimiter=';')
        headers = next(reader, None)
        if measure and any(h.endswith(measure) for h in headers):
            print(f"Measurement '{measure}' already present at the end of a column header. Skipping modification.")
            return
        if headers is None:
            return False

        # Check if any column already has the measurement appended
        if measure and any(measure in h for h in headers):
            print(f"Measurement '{measure}' already appended to a column header. Skipping modification.")
            return
        ...
        if measure and "Total" in headers:
            print(f'Appending measurement "{measure}" to "Total" column header.')
                    ...
                    with open(file_path, mode='r', encoding='utf-8', errors='replace') as csvfile:
                        ...
                        for row in reader:
                            ...
                            if measure_set_index is not None:
                                row[measure_set_index] = f"{row[measure_set_index]} {measure}"
...
def run(csv_file_path, measure):
    ...
    print(f"Processing file: {csv_file_path} with measure: {measure}")
    ...
    start_time = time.time()
    graph = morph_kgc.materialize('config_CSV.ini')
