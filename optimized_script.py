...
def add_POM_from_csv(file_path, measure):
    dimensions = Graph().parse("inelod-voc-dimension.ttl", format="turtle")
    measures = Graph().parse("inelod-voc-measure.ttl", format="turtle")
    file_name = os.path.splitext(os.path.basename(file_path))[0]
    dsd_uri = INELOD[file_name + "_dsd"]
    detect_and_replace_measures(file_path, measure)
    ...
    with open(file_path, mode='r', encoding='utf-8') as csvfile:
        ...
        for column in columns:
            ...
            if not res_dim.askAnswer and not res_measure.askAnswer and res_measure_set.askAnswer:
                ...
                g_mappings.add((pom_bnode, RR.objectMap, object_bnode))
                if measure :
                    g_mappings.add((object_bnode, RML.reference, Literal(measure)))
                else:
...
def detect_and_replace_measures(file_path, measure):
    import csv as _csv
    ...
    # Load measures vocabulary (same as your original logic)
    medidas = _rdflib.Graph().parse("inelod-voc-measure.ttl", format="ttl")
    ...
    # 1) If any header already ends with the selected measure, do nothing
    if measure and any(isinstance(h, str) and h.endswith(measure) for h in headers):
        return
    
    
    # 2) Apply the original "Total" -> measure rename FIRST
    if measure and "Total" in headers:
        print(f"Renaming 'Total' column to '{measure}'")
        headers = [measure if h == "Total" else h for h in headers]

    # 3) Find the Medida column (if present)
    measure_set_index = headers.index('Medida') if ('Medida' in headers) else None

    # 4) If no rename happened and there's no Medida column, nothing to do
    #    (we still might have renamed 'Total'; if so, we must rewrite the file)
    need_header_rewrite_only = (measure_set_index is None)
    ...
    try:
        with open(file_path, mode='r', encoding='utf-8', errors='replace', newline='') as _src, \
            ...
            _old_headers = next(_reader, None)  # discard old header
            _writer.writerow(headers)           # write updated header (possibly unchanged)

            # If there is a Medida column, do the per-row SPARQL replacement
            if measure_set_index is not None and measure:
                for row in _reader:
                    if measure_set_index < len(row):
                        cell_value = row[measure_set_index]
                        if cell_value and not (isinstance(cell_value, str) and cell_value.endswith(measure)):
                            measure_replace_query = (
                                ' PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>'
                                ' PREFIX ine: <https://stats.linkeddata.es/voc/cubes/vocabulary#>'
                                f' SELECT ?measure WHERE {{ ?measure a ine:MeasureProperty ; rdfs:label "{cell_value}"@es . }}'
                            )
                ...
                # No Medida replacement—just copy rows after writing possibly-updated header
                for row in _reader:
                    _writer.writerow(row)
...
def run(csv_file_path, measure):
    ...
    add_INE_metadata(csv_file_path, measure)
    print(f"INE metadata added in {time.time() - start_time:.2f} seconds")
    ...
    start_time = time.time()
    add_POM_from_csv(csv_file_path, measure)
    ...
    try:
        result = subprocess.run(
            ["python","-m","morph_kgc", "config_CSV.ini"],
            check=True,
            capture_output=True,
            text=True
        )
        ...
        print("morph-kgc CLI failed:", e.stderr or str(e))
        raise
